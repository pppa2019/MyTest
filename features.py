...
def wam(sentence, w2v_model, method='mean', aggregate=True):
    ...
    arr = [w2v_model[k] if k in w2v_model.vocab else np.zeros(300) for k in word_list]
    arr = np.array(arr)
...
def Find_Label_embedding(example_matrix, label_embedding, method='mean'):
    ...
    similarity_matrix = [[distance.cosine(label_vec, word_vec) for label_vec in label_embedding] for word_vec in example_matrix]
    #
    ...
    #
    #
    attention = np.mean(similarity_matrix, axis=1)#是不是应该归一化？
    attention = softmax(attention)
    # 将样本的词嵌入与注意力机制相乘得到
    attention_embedding = (np.array(example_matrix).T * attention).T
    if method == 'mean':
...
def Find_embedding_with_windows(embedding_matrix, window_size=2,
    ...
    # 最终的词向量
    result_list = []
    if len(embedding_matrix)<window_size:
        window_size = len(embedding_matrix)
    #######################################################################
    #          TODO:  句向量处理 #
    #######################################################################
    ...
    for k1 in range(len(embedding_matrix)):
        ...
        if int(k1 + window_size) > len(embedding_matrix):
            ...
            #
            extract_embedding = embedding_matrix[k1:k1+window_size+1, :]
            result_list.append(np.mean(extract_embedding, axis=0))
...
def get_lda_features_helper(lda_model, document):
    ...
    # topic_partition = lda_model[document]
